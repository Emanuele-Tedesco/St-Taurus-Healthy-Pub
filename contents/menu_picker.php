...
<div class="col-sm-8">
	...
	<?php //genero la tabella
		$query = "SELECT c.id as type, b.id as subtype, titolo, a.descrizione as descrizione, prezzo, light, vegan, lower(substring_index(b.descrizione,' ',-1)) as descrMenu 
		...
		$result = get_resultset($conn, $query);
		if ($result->num_rows) {
			echo '<div class="tab-pane"><table class="table table-striped"><thead><tr class="info"><th>Nome</th><th></th><th>Prezzo</th><th>LightFood</th><th>Vegan</th></tr></thead><tbody>';
			...
			while ($row = mysqli_fetch_assoc($result)) {
				...
				echo '</td><td>'.$p.' €</td><td>'.$l.'</td><td>'.$v.'</td></tr>';
			}
